...
## Curation scripts → [`curation/`](curation/README.md)

Four scripts that *aren't part of the standard pipeline run* live in the [`curation/`](curation/) subdirectory and are documented separately. They are invoked on demand rather than as part of every regeneration:

| Script | Purpose |
| --- | --- |
| `audit_language_identifiers.py` | Read-only diagnostic for the BCP 47 and service-support columns. Run after a regen to sanity-check the columns. |
| `refresh_service_support.py` | Refresh `service_language_code_support.csv` from Google Translate v3 API. Run when Google adds/removes language support. |
| `spotcheck_bcp47_codes.py` | One-off validation that BCP 47 normalization produces identical Google MT output — committed result lives at `bcp47_spotcheck_results.csv`. |
| `generate_family_assignments.py` | One-time generator for `language_family_assignments.json`. Re-run only when revising per-code family decisions. |

Invocation forms are `python -m scripts.experiment.curation.<script>`. See [`curation/README.md`](curation/README.md) for per-script flags, outcome semantics, and methodology notes.
