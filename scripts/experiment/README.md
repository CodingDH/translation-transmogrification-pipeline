...
```bash
python scripts/experiment/audit_language_identifiers.py datasets/metadata_files/language_codes_comprehensive.csv
```

Use this after re-running `generate_language_codes.py` or refreshing the service-support manifest to confirm the columns look right before committing the regenerated CSV. The script uses `na_filter=False` to keep the literal string `'nan'` (Min Nan Chinese, ISO 639-3) as a string rather than silently converting it to NaN.
...
python -m scripts.experiment.refresh_service_support --dry-run   # print diff, don't write
python -m scripts.experiment.refresh_service_support --no-backup # skip the backup file
```
...
This is an *additive* experiment, not a rerun: the pipeline's 880 translations remain keyed off `language_code` (the community/registry identifier). The script answers the methodological question "does normalization change anything operationally?"
