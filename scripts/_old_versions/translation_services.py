...
claude_client = None  # Claude client
gemini_client = None
GEMINI_AVAILABLE = False
GEMINI_MODEL = None
LINGVANEX_AVAILABLE = False
model = None  # EasyNMT model
current_prompt_variant = 'comparative'
current_term_contexts = {}

def set_globals(console_obj, translate_client_obj, openai_client, claude_client_obj, 
                gemini_client_obj, gemini_available, gemini_model, 
                lingvanex_available, enmt_model, prompt_variant, term_contexts):
    """Set global objects needed by translation services."""
    global console, translate_client, client, claude_client, gemini_client
    global GEMINI_AVAILABLE, GEMINI_MODEL, LINGVANEX_AVAILABLE, model
    global current_prompt_variant, current_term_contexts
    
    console = console_obj
    translate_client = translate_client_obj
    client = openai_client
    claude_client = claude_client_obj
    gemini_client = gemini_client_obj
    GEMINI_AVAILABLE = gemini_available
    GEMINI_MODEL = gemini_model
    LINGVANEX_AVAILABLE = lingvanex_available
    model = enmt_model
    current_prompt_variant = prompt_variant
    current_term_contexts = term_contexts

def get_gt_translation(row: pd.Series, error_file_path: str) -> pd.Series:
	"""
...
def get_enmt_translation(row: pd.Series, error_file_path: str) -> pd.Series:
	"""
...
def get_openai_translation(row: pd.Series, error_file_path: str) -> pd.Series:
	"""
...
def get_claude_translation(row: pd.Series, error_file_path: str) -> pd.Series:
	"""
...
def get_gemini_translation(row: pd.Series, error_file_path: str) -> pd.Series:
	"""
...
def get_lingvanex_translation(row: pd.Series, error_file_path: str) -> pd.Series:
	"""
...
def get_ollama_translation(row: pd.Series, error_file_path: str, ollama_model: str = 'llama3.1', request_delay: float = 2.0, request_timeout: int = 120, consecutive_timeouts: int = 0) -> Tuple[pd.Series, int]:
	"""
	...
	return row, consecutive_timeouts
def check_if_wikipedia_page_exists(term_source: str, error_file_path:str ) -> dict:
	"""
