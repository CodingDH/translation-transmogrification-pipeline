...
| `family_name_reconciled` | str | **Glottolog 5.3 cross-validated family.** Equals `family_name` when ISO 639-5 and Glottolog agree (586 codes); equals Glottolog's classification where ISO 639-5 uses geographic groupings (`Caucasian`, `North American Indian`) or abandoned/contested macrofamilies (`Niger-Kordofanian`, `Altaic`) — 295 reclassifications. Per-pair decisions live in `family_reconciliation.csv`. This is the pre-review reconciled layer; downstream `get_language_family()` calls prefer `language_codes_comprehensive_family_reviewed.csv` when present, using `family_name_analysis` before falling back to this column. |
...
### `language_codes_comprehensive_family_reviewed.csv` — reviewed family metadata layer

Generated by `python scripts/exploration/apply_family_review_metadata.py`. This file starts from `language_codes_comprehensive.csv` but does **not** rebuild the language list. It applies the reviewed family metadata after the fact, preserving the generated source-build artifact while adding reviewed analytical columns.

Key added columns:

- `family_name_reconciled_pre_review` — copy of the generated `family_name_reconciled` value.
- `family_name_reconciled_reviewed` — reviewed family assignment after `family_reconciliation_reviewed.csv` is applied, when present.
- `family_name_analysis` — analysis/chart label from `family_analysis_mapping_reviewed.csv` when present, otherwise the generated mapping.
- `family_review_source` and `family_review_notes` — provenance for reviewed overrides.
- `family_name_analysis_source` — mapping file used to create the analysis label.

Downstream `get_language_family()` calls prefer this reviewed file when it exists, using `family_name_analysis` first and falling back through the reviewed/pre-review family columns.
...
### `family_reconciliation_review.csv` — unified family review queue

Generated by `python scripts/exploration/build_family_reconciliation_review.py`. Contains both ISO/Glottolog disagreement-pair rows (`review_kind == "iso_glottolog_pair"`) and JSON fallback language rows (`review_kind == "json_fallback_language"`). The JSON fallback rows are the cases where `language_family_assignments.json` wins the raw family-assignment priority order because no manual `MANUAL_LANG_TO_SET5` mapping applies.

The file adds `glottolog_validation_status`, a non-destructive coherence suggestion layer (`coherence_issue`, `suggested_iso639_5`, `suggested_family_name`, `suggestion_confidence`, `suggestion_reason`), `review_tier`, editable review columns (`reviewer_decision`, `reviewer_family_name`, `reviewer_iso639_5`, `reviewer_notes`), and analysis-policy annotations from `family_analysis_mapping.csv` or `family_analysis_mapping_reviewed.csv` when present. The coherence suggestions are review prompts for cases such as abandoned macrofamilies, geographic groupings, and known direct-assignment errors; they do not change `family_name_reconciled` until reviewed.

Review this file in `html_files/family_reconciliation_reviewer.html`. The reviewer downloads edits as `family_reconciliation_reviewed.csv`. If that reviewed file is saved in this directory, `apply_family_review_metadata.py` applies reviewed pair decisions and rows where `review_kind == "json_fallback_language"`, `reviewer_decision == "revise"`, and `reviewer_family_name` is non-empty. This writes `language_codes_comprehensive_family_reviewed.csv`, changing the reviewed family columns without overwriting the raw `family_name` source trace or rebuilding the language-code list.

### `family_analysis_mapping.csv` — review queue for analysis family labels

Generated by `python scripts/exploration/build_family_analysis_mapping.py`. Contains one row per current `family_name_reconciled` value and proposes a `family_name_analysis` label for aggregate charts. The proposal preserves the reconciled label by default and uses the reviewer only for label-level decisions such as obvious spelling variants, missing labels, and special non-family categories. Problematic macrofamily or geographic labels are marked `row_level_reconciliation` and handled later in `family_reconciliation_review.csv`, not renamed here. Columns include `family_name_reconciled`, `family_name_analysis`, `n_languages`, `sample_languages`, `raw_family_names`, `mapping_action`, `mapping_reason`, `review_status`, and `reviewer_notes`.

Review this file in `html_files/family_analysis_mapping_reviewer.html`. The reviewer downloads edits as `family_analysis_mapping_reviewed.csv`. The reviewed mapping is consumed by `apply_family_review_metadata.py` to produce a durable `family_name_analysis` column while preserving `family_name` and the pre-review `family_name_reconciled`.
